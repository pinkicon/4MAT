...
def partA(): 
    if not  session["id"] : 
        ...
        if request.method == 'POST':   
            ...
            return render_template('partA.html',  examples = executeQuery('''SELECT exampleNo, exampleTitle  From t_example where exampleType='A' order by exampleNo asc limit 10'''), choices  = executeQuery('''SELECT A.exampleNo, A. exampleTItle  , B.Title ChoiceCase , B.ChoiceCaseNo, B.FiqureType  FROM t_example A, t_choicecase B  WHERE A.exampleNo = B.exampleNo order by A.exampleNo asc '''))
...
def graph():  
    if not  session["id"] : 
        ...
        Items = executeQuery('''  SELECT SUM(A.Point) POINT  , C.FiqureType FigureType  FROM t_answerchoice A, t_choiceCase C  WHERE A.ChoiceCaseNo = C.ChoiceCaseNo GROUP BY C.FiqureType''') 
